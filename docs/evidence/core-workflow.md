...
4. The issue can move through the status workflow:
   - `submitted`
   - `in_review`
   - `assigned`
   - `resolved`
5. Each status change is written to `issue_status_history`.
6. The frontend shows the status history for the selected report.
